...
export default function WishlistScreen({ navigation }: any) {
  ...
  const handleAddAllToCart = useCallback(() => {
    ...
    Toast.show({ type: 'success', text1: 'Added All to Cart', text2: `${wishlistItems.length} items added!` });
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { borderBottomColor: colors.border, borderBottomWidth: 1 }]}>
        <View>
          ...
          {wishlistItems.length > 0 && (
            <Text style={[styles.headerSub, { color: colors.textSecondary, fontFamily: fonts.regular }]}>
        ...
        {/* Total Value Chip */}
        {wishlistItems.length > 0 && (
          <View style={[styles.totalChip, { backgroundColor: colors.primary + '15', borderColor: colors.primary + '30' }]}>
            <Ionicons name="pricetag-outline" size={14} color={colors.primary} />
            <Text style={[styles.totalChipText, { color: colors.primary, fontFamily: fonts.bold }]}>
      ...
      {wishlistItems.length > 0 && (
        <>
          {/* ── Sort Bar ── */}
          <View style={[styles.sortBar, { borderBottomColor: colors.border, borderBottomWidth: 1 }]}>
            <Text style={[styles.sortLabel, { color: colors.textSecondary, fontFamily: fonts.medium }]}>Sort by:</Text>
                  ...
                  <Text
                    style={[
                      styles.sortChipText,
                      {
                        ...
                        fontFamily: sortBy === opt.key ? fonts.semiBold : fonts.regular,
                      },
                    ]}
                  >
                    {opt.label}
          ...
          <View style={[styles.bottomBar, { backgroundColor: colors.card, borderTopColor: colors.border, borderTopWidth: 1 }]}>
            <Button
