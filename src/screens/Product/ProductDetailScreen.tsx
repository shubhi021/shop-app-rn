...
// ─── Mock reviews ──────────────────────────────────────────────────────────────
const MOCK_REVIEWS = [
  { id: '1', author: 'Sarah M.', rating: 5, date: 'Jan 2025', comment: 'Absolutely love it! Quality exceeded my expectations. Fast delivery and beautiful packaging.' },
  { id: '2', author: 'Thomas K.', rating: 4, date: 'Dec 2024', comment: 'Great product for the price. Minor packaging issue but the item itself is perfect.' },
  { id: '3', author: 'Lena W.', rating: 5, date: 'Nov 2024', comment: 'Bought this as a gift and everyone was impressed. Will definitely order again!' },
];
    ...
    <View style={srStyles.container}>
      {breakdown.map(row => (
        <View key={row.stars} style={srStyles.row}>
          <Text style={[srStyles.starNum, { color: colors.textSecondary, fontFamily: fonts.medium }]}>{row.stars}</Text>
          <Ionicons name="star" size={10} color="#F59E0B" />
          <View style={[srStyles.barBg, { backgroundColor: colors.border }]}>
            <View style={[srStyles.barFill, { backgroundColor: '#F59E0B', width: `${Math.round(row.pct * 100)}%` as any }]} />
          </View>
          <Text style={[srStyles.pctText, { color: colors.textTertiary, fontFamily: fonts.regular }]}>{Math.round(row.pct * 100)}%</Text>
        </View>
      ))}
    </View>
  );
...
const srStyles = StyleSheet.create({
  container: { width: '100%' },
  row: { flexDirection: 'row', alignItems: 'center', marginBottom: hp(0.6), gap: wp(1.5) },
...
export default function ProductDetailScreen({ route, navigation }: any) {
    ...
    const fetchProduct = async () => {
      ...
      try {
        ...
        try {
          ...
          setRelatedProducts(all.filter((p: Product) => p.id !== productId).slice(0, 6));
        } catch {
  ...
  const handleShare = useCallback(async () => {
    ...
    try {
      await Share.share({
        title: product.title,
        message: `Check out "${product.title}" for ${formatCurrency(product.price)} on ShopApp! 🛍️`,
      });
    } catch {
      // ignore
    }
  ...
  const handleWishlistToggle = () => {
    ...
    if (isWishlisted) {
      ...
      Toast.show({ type: 'info', text1: 'Removed from Wishlist', text2: `${product.title.substring(0, 20)}... removed.` });
    } else {
      dispatch(addToWishlist(product));
      Toast.show({ type: 'success', text1: 'Added to Wishlist', text2: `${product.title.substring(0, 20)}... added!` });
    }
  };
  ...
  const handleAddToCart = () => {
    ...
    if (quantity > 1) dispatch(updateQuantity({ productId: product.id, quantity }));
    Toast.show({ type: 'success', text1: 'Added to Cart', text2: `${quantity} × ${product.title.substring(0, 20)}... added!` });
  };
  ...
  if (error || !product) {
    return (
      <View style={[styles.errorContainer, { backgroundColor: colors.background }]}>
        ...
        <Text style={[styles.errorText, { color: colors.text, fontFamily: fonts.bold }]}>Error Loading Product</Text>
        <Text style={[styles.errorSub, { color: colors.textSecondary, fontFamily: fonts.regular }]}>{error || 'Product not found.'}</Text>
  ...
  const isClothing = product.category?.includes('clothing') || product.category?.includes('women') || product.category?.includes('men');
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        <Animated.Text
          style={[styles.headerTitle, { color: colors.text, fontFamily: fonts.bold, opacity: headerTitleOpacity }]}
          numberOfLines={1}
        >
          {product.title}
        </Animated.Text>
      ...
      <Animated.ScrollView
        ...
        contentContainerStyle={[styles.scrollContent, { paddingTop: hp(7.5) + insets.top, paddingBottom: hp(16) + insets.bottom }]}
      ...
      >
        ...
        <View style={[styles.heroImageContainer, { backgroundColor: '#FFFFFF' }]}>
          ...
          <View style={[styles.stockBadge, { backgroundColor: inStock ? '#DCFCE7' : '#FEE2E2' }]}>
            ...
            <Text style={[styles.stockText, { color: inStock ? '#16A34A' : '#DC2626', fontFamily: fonts.semiBold }]}>
          ...
          <Animated.Image
            ...
            style={[styles.heroImage, { transform: [{ scale: imageScale }, { translateY: imageTranslateY }] }]}
            resizeMode="contain"
          />
        </View>
        ...
        <View style={styles.infoSection}>
          ...
          <Text style={[styles.title, { color: colors.text, fontFamily: fonts.bold }]}>{product.title}</Text>
          ...
          <View style={styles.ratingRowContainer}>
            ...
            <TouchableOpacity onPress={() => {}}>
              <Text style={[styles.ratingCount, { color: colors.primary, fontFamily: fonts.medium }]}>
          ...
          {isClothing && (
            <>
              <View style={styles.selectorHeader}>
                <Text style={[styles.sectionTitle, { color: colors.text, fontFamily: fonts.semiBold }]}>Size</Text>
                <TouchableOpacity>
                  <Text style={[styles.sizeGuide, { color: colors.primary, fontFamily: fonts.medium }]}>Size Guide →</Text>
                </TouchableOpacity>
              </View>
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.chipScrollRow}>
                {SIZE_OPTIONS.map(size => (
                  <TouchableOpacity
                    ...
                    activeOpacity={0.8}
                  >
                    <Text style={[styles.sizeChipText, { color: selectedSize === size ? '#FFF' : colors.text, fontFamily: fonts.semiBold }]}>
                      {size}
                    </Text>
                  </TouchableOpacity>
                ))}
              ...
              {/* Color Selector */}
              <Text style={[styles.sectionTitle, { color: colors.text, fontFamily: fonts.semiBold, marginBottom: hp(1.2) }]}>Color</Text>
              <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.chipScrollRow}>
                {COLOR_OPTIONS.map(col => (
                  <TouchableOpacity
                    ...
                    style={[
                      ...
                      { borderColor: selectedColor === col.value ? colors.primary : 'transparent' },
                    ]}
                    activeOpacity={0.8}
                  >
                    <View style={[styles.colorDot, { backgroundColor: col.value }]} />
                    <Text style={[styles.colorLabel, { color: colors.textSecondary, fontFamily: fonts.regular }]}>{col.label}</Text>
                  </TouchableOpacity>
                ))}
              </ScrollView>
              <View style={[styles.sectionDivider, { backgroundColor: colors.border }]} />
          ...
          <View style={styles.highlightsContainer}>
            <View style={[styles.highlightCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
              ...
              <Text style={[styles.highlightTitle, { color: colors.text, fontFamily: fonts.semiBold }]}>{t('freeShippingLabel') || 'Free Shipping'}</Text>
              <Text style={[styles.highlightSubtitle, { color: colors.textSecondary, fontFamily: fonts.regular }]}>{t('freeShippingDesc') || 'On orders over €39'}</Text>
            </View>
            <View style={[styles.highlightCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
              <Ionicons name="shield-checkmark-outline" size={20} color={colors.primary} />
              <Text style={[styles.highlightTitle, { color: colors.text, fontFamily: fonts.semiBold }]}>{t('secureCheckout') || 'Secure Pay'}</Text>
              <Text style={[styles.highlightSubtitle, { color: colors.textSecondary, fontFamily: fonts.regular }]}>{t('sslEncrypted') || '100% Secure'}</Text>
            </View>
            <View style={[styles.highlightCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
              <Ionicons name="refresh-circle-outline" size={20} color={colors.primary} />
              <Text style={[styles.highlightTitle, { color: colors.text, fontFamily: fonts.semiBold }]}>{t('easyReturns') || 'Easy Returns'}</Text>
              <Text style={[styles.highlightSubtitle, { color: colors.textSecondary, fontFamily: fonts.regular }]}>{t('returnWindow') || '30-Day Window'}</Text>
            </View>
          </View>
          ...
          <TouchableOpacity onPress={() => setDescExpanded(v => !v)} style={styles.readMoreBtn} activeOpacity={0.7}>
          ...
          <View style={styles.reviewsHeader}>
            <Text style={[styles.sectionTitle, { color: colors.text, fontFamily: fonts.semiBold }]}>Reviews</Text>
            <View style={[styles.overallRatingChip, { backgroundColor: colors.warning + '18' }]}>
              <Ionicons name="star" size={14} color={colors.warning} />
              <Text style={[styles.overallRatingText, { color: colors.text, fontFamily: fonts.bold }]}>
              ...
              >
                <View style={styles.reviewTop}>
                  <View style={[styles.reviewAvatar, { backgroundColor: colors.primary + '20' }]}>
                    <Text style={[styles.reviewAvatarText, { color: colors.primary, fontFamily: fonts.bold }]}>
                      {rev.author[0]}
                    </Text>
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={[styles.reviewAuthor, { color: colors.text, fontFamily: fonts.semiBold }]}>{rev.author}</Text>
                    <Text style={[styles.reviewDate, { color: colors.textTertiary, fontFamily: fonts.regular }]}>{rev.date}</Text>
                  </View>
                ...
                <Text style={[styles.reviewComment, { color: colors.textSecondary, fontFamily: fonts.regular }]}>
                  {rev.comment}
                </Text>
              </View>
            ))}
          ...
          {relatedProducts.length > 0 && (
            <>
              <View style={[styles.sectionDivider, { backgroundColor: colors.border }]} />
              <Text style={[styles.sectionTitle, { color: colors.text, fontFamily: fonts.semiBold, marginBottom: hp(1.5) }]}>
                You May Also Like
              </Text>
              <FlatList
                ...
                renderItem={({ item }) => (
                  ...
                  >
                    <View style={styles.relatedImageBox}>
                      <Animated.Image source={{ uri: item.image }} style={styles.relatedImage} resizeMode="contain" />
                    </View>
                    <Text style={[styles.relatedTitle, { color: colors.text, fontFamily: fonts.semiBold }]} numberOfLines={2}>
                      {item.title}
                    </Text>
                    <Text style={[styles.relatedPrice, { color: colors.primary, fontFamily: fonts.bold }]}>
      ...
      >
        <View style={styles.totalRow}>
          <Text style={[styles.totalLabel, { color: colors.textSecondary, fontFamily: fonts.regular }]}>Total</Text>
          <Text style={[styles.totalPrice, { color: colors.primary, fontFamily: fonts.bold }]}>
...
const styles = StyleSheet.create({
  ...
  colorDot: { width: wp(8.0), height: wp(8.0), borderRadius: wp(4.0), marginBottom: hp(0.4) },
  ...
  highlightSubtitle: { fontSize: fp(2.67), marginTop: hp(0.2), textAlign: 'center' },
  ...
  reviewTop: { flexDirection: 'row', alignItems: 'center', marginBottom: hp(1.0), gap: wp(3.0) },
  reviewAvatar: {
    width: wp(9.0),
    height: wp(9.0),
    borderRadius: wp(4.5),
  ...
  errorContainer: { flex: 1, justifyContent: 'center', alignItems: 'center', paddingHorizontal: wp(8.53) },
