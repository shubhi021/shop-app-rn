...
const MOCK_NOTIFICATIONS: NotificationItem[] = [
  { id: '1', type: 'impact', titleKey: 'notifImpactTitle', msgKey: 'notifImpactMsg', timeKey: 'notifTime2h', isRead: false, group: 'today' },
  { id: '2', type: 'order', titleKey: 'notifOrderTitle', msgKey: 'notifOrderMsg', timeKey: 'notifTime5h', isRead: false, group: 'today' },
  { id: '3', type: 'promo', titleKey: 'notifPromoTitle', msgKey: 'notifPromoMsg', timeKey: 'notifTime1d', isRead: false, group: 'today' },
  { id: '4', type: 'price', titleKey: 'notifPriceTitle', msgKey: 'notifPriceMsg', timeKey: 'notifTime2d', isRead: true, group: 'earlier' },
  { id: '5', type: 'system', titleKey: 'notifSystemTitle', msgKey: 'notifSystemMsg', timeKey: 'notifTime5d', isRead: true, group: 'earlier' },
];
...
export default function NotificationScreen({ navigation }: any) {
  ...
  const getIconConfig = (type: NotificationItem['type']) => {
    switch (type) {
      case 'impact': return { name: 'leaf', color: '#10B981', bgColor: '#E6FBF3' };
      case 'order': return { name: 'cube', color: '#3B82F6', bgColor: '#EFF6FF' };
      case 'promo': return { name: 'flash', color: '#F59E0B', bgColor: '#FEF3C7' };
      case 'price': return { name: 'star', color: '#EC4899', bgColor: '#FDF2F8' };
      default: return { name: 'shield-checkmark', color: '#6B7280', bgColor: '#F3F4F6' };
    }
  };
  ...
  const renderItem = ({ item, index }: { item: NotificationItem; index: number }) => {
    ...
    Animated.parallel([
      Animated.timing(itemFade, { toValue: 1, duration: 300, delay: index * 60, useNativeDriver: true }),
      Animated.timing(itemSlide, { toValue: 0, duration: 300, delay: index * 60, useNativeDriver: true }),
    ]).start();
    ...
    return (
      <Animated.View style={{ opacity: itemFade, transform: [{ translateY: itemSlide }] }}>
        ...
        >
          ...
          <View style={[styles.iconContainer, { backgroundColor: isDark ? '#1F293D' : iconConfig.bgColor }]}>
            <Ionicons name={iconConfig.name as any} size={18} color={isDark ? colors.primary : iconConfig.color} />
          </View>

          <View style={styles.contentContainer}>
            <View style={styles.textRow}>
              <Text
                style={[styles.notifTitle, { color: colors.text, fontFamily: item.isRead ? fonts.medium : fonts.bold }]}
                numberOfLines={1}
              >
                {t(item.titleKey)}
              </Text>
            ...
            <Text
              style={[styles.message, { color: item.isRead ? colors.textSecondary : colors.text, fontFamily: fonts.regular }]}
            >
              {t(item.msgKey)}
            </Text>
            ...
            {item.type === 'order' && !item.isRead && (
              <View style={[styles.actionChip, { backgroundColor: colors.primary + '15' }]}>
                <Text style={[styles.actionChipText, { color: colors.primary, fontFamily: fonts.semiBold }]}>Track Order →</Text>
              </View>
            )}
            {item.type === 'promo' && !item.isRead && (
              <View style={[styles.actionChip, { backgroundColor: '#F59E0B18' }]}>
                <Text style={[styles.actionChipText, { color: '#D97706', fontFamily: fonts.semiBold }]}>Shop Now →</Text>
              </View>
            )}
          </View>
    ...
    return (
      <>
        <View style={styles.sectionHeader}>
          <Text style={[styles.sectionHeaderText, { color: colors.textTertiary, fontFamily: fonts.semiBold }]}>{title}</Text>
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { borderBottomColor: colors.border }]}>
        <View style={styles.headerLeft}>
          ...
          <View>
            ...
            {unreadCount > 0 && (
              <Text style={[styles.unreadLabel, { color: colors.primary, fontFamily: fonts.medium }]}>
                {unreadCount} unread
              </Text>
            )}
          </View>
        ...
        <View style={styles.headerActions}>
          {notifications.length > 0 && (
            <TouchableOpacity onPress={clearAll} activeOpacity={0.7} style={styles.headerActionBtn}>
              <Text style={[styles.actionText, { color: '#EF4444', fontFamily: fonts.medium }]}>Clear</Text>
            </TouchableOpacity>
          )}
          {hasUnread && (
            <TouchableOpacity onPress={markAllAsRead} activeOpacity={0.7} style={styles.headerActionBtn}>
      ...
      {/* ── Filter Chips ── */}
      <View style={[styles.filterBar, { borderBottomColor: colors.border, borderBottomWidth: 1 }]}>
        {FILTER_OPTIONS.map(opt => {
          const count = opt.key === 'all' ? notifications.length : notifications.filter(n => n.type === opt.key).length;
          return (
            <TouchableOpacity
              key={opt.key}
              ...
              style={[
                styles.filterChip,
                {
                  backgroundColor: activeFilter === opt.key ? colors.primary : colors.card,
                  borderColor: activeFilter === opt.key ? colors.primary : colors.border,
                },
              ]}
              activeOpacity={0.8}
            >
              ...
              <Text
                style={[
                  ...
                  {
                    color: activeFilter === opt.key ? '#FFF' : colors.textSecondary,
                    fontFamily: activeFilter === opt.key ? fonts.semiBold : fonts.regular,
                  },
                ]}
              >
                {opt.label}
              ...
              {count > 0 && (
                <View
                  style={[
                    styles.filterCount,
                    { backgroundColor: activeFilter === opt.key ? '#FFFFFF30' : colors.border },
                  ]}
                >
                  <Text
                    style={[
                      styles.filterCountText,
                      { color: activeFilter === opt.key ? '#FFF' : colors.textSecondary, fontFamily: fonts.bold },
                    ]}
                  >
                    {count}
                  </Text>
        ...
        {filtered.length > 0 ? (
          ...
          <View style={styles.emptyContainer}>
            ...
            <Text style={[styles.emptySubtitle, { color: colors.textSecondary, fontFamily: fonts.regular }]}>
              {activeFilter === 'all' ? t('emptyNotificationsSubtitle') : `No ${activeFilter} notifications.`}
            </Text>
          </View>
        )}
      </Animated.View>
...
const styles = StyleSheet.create({
  ...
  sectionHeaderText: { fontSize: fp(3.2), letterSpacing: 0.5, textTransform: 'uppercase' },
  ...
  emptySubtitle: { fontSize: fp(3.47), textAlign: 'center', lineHeight: hp(2.46) },
});
